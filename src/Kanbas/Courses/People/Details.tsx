...
export default function PeopleDetails() {
  ...
  const [edit_role, setEditRole] = useState(false);
    ...
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <div className="text-danger fs-4 wd-name"> 
        ...
        <span className="wd-section">     
            {!edit_email && (
                <FaPencil onClick={() => setEditEmail(true)}
                    className="float-end mt-2 wd-edit" /> )}
            {edit_email && (
                <FaCheck onClick={() => saveEmail()}
                    className="float-end mt-2 me-2 wd-save" /> )}  
            {!edit_email && (
                <div className="wd-name"
      ...
      <b>Role:</b>         
            <span className="wd-section">  
                {!edit_role && (
                    <FaPencil onClick={() => setEditRole(true)}
                        className="float-end wd-edit" /> )}
                {edit_role && (
                    <FaCheck onClick={() => saveRole()}
                        className="float-end wd-save" /> )}  
                {!edit_role && (
                    <div className="wd-name"
